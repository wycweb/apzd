...
<div class=" clearfix" style="width:100%">
    <div id="add_brand" class="clearfix" style="width:100%">
        ...
        <div class="title_name" style="width:100%">动态修改</div>
        <ul class="add_conent">
            <input type="hidden" name="id" value="<?php echo $data['id'];?>">
            <input type="hidden" name="old_img" value="<?php echo $data['active_img_url'];?>">

            <li class=" clearfix"><label class="label_name">动态标题：</label> <input name="active_name" type="text" value="<?php echo $data['active_name']; ?>" class="add_text"/></li>
            <li class=" clearfix"><label class="label_name">动态图片：</label>
                <div class="demo l_f">
                   <div class="logobox">
                       <div class="resizebox" style="width:<?php echo C('ACTIVE_IMAGE_WIDTH');?>px;height:<?php echo C('ACTIVE_IMAGE_HEIGHT'); ?>px;">
                           <img src="<?php echo IMG_URL.$data['active_img_url']?>" width="100%" height="100%" />
                       </div>
                       <input id="pic" type="file" name="active_img_url" />
                   </div>
                   <div class="prompt"><p>图片大小<b><?php echo C('ACTIVE_IMAGE_WIDTH');?>px*<?php echo C('ACTIVE_IMAGE_HEIGHT');?>px</b>图片大小小于10MB,</p><p>支持.jpg;.gif;.png;.jpeg格式的图片</p></div>
                </div>
            </li>
            <li class=" clearfix"><label class="label_name">状态：</label>
                <label><input <?php echo $data['active_show_status'] == '启用' ? "checked='checked'" : "";?> name="active_show_status" type="radio" class="ace" value="启用"><span class="lbl">启用</span></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <label><input <?php echo $data['active_show_status'] == '停用' ? "checked='checked'" : "";?> type="radio" class="ace" name="active_show_status" value="停用"><span class="lbl">停用</span></label>
            </li>
...
$('#submit').click(function(){
    ...
    $.ajax({
        type : 'post',
        url : "/apzd/index.php/Home/Active/save",
        data : form,
        ...
        success : function (response){
            ...
            if(response.status){
                $('#submit').val('修改');
                layer.msg(response.result,{icon:1,time:1000},function (){location.href="/apzd/index.php/Home/Active/lst";$('#submit').attr('disabled',false);});
            }else{
