  ...
  <div id="brand_style">
    <div class="search_style">
      ...
      <ul class="search_content clearfix">
          <form action="/apzd/apzd/index.php/Home/Employ/lst">
       <li><label class="l_f">职位名称</label><input name="search" type="text"  class="text_add" placeholder="输入招聘名称" value="<?php echo $_GET[search]?>" style=" width:250px"/></li>
     ...
     <div class="border clearfix">
       <span class="l_f">
        <a href="/apzd/apzd/index.php/Home/Employ/add" title="添加招聘" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加招聘</a>

        <a href="/apzd/apzd/index.php/Home/Employ/lst" class="btn btn-success">全部招聘</a>
       </span>
       <span class="r_f">共：<?php echo count($adData);?>个招聘</span>
     </div>
      ...
      <div class="table_menu_list" style="width:100%">
          <table class="table table-striped table-bordered table-hover" id="sample-table">
              ...
              <tr>
                  ...
                  <th width="200px">工作职责</th>
                  <th width="200px">任职要求</th>
              ...
              <tbody>
              <?php foreach($jobData as $k => $v): ?>
                  <tr>
                  <td width="80px"><?php echo $v['id']; ?></td>
                  <td width="50px"><input type="text"  onblur="sort(this,'<?php echo $v[id];?>')" sort="<?php echo $v['job_sort'];?>" class="input-text text-c" value="<?php echo $v['job_sort']?>" style="width:60px"></td>
                  <td><?php echo $v['job_name']; ?></td>
                  <td style="text-align: left;">
                      <?php
 $arr = explode(',',$v['job_duty']); foreach($arr as $k1 => $v1){ $num = $k1+1; echo $num.'、'.$v1.'<br>'; } ?>

                  </td>
                      <td style="text-align: left;">
                      <?php
 $arr1 = explode(',',$v['job_require']); foreach($arr1 as $k1 => $v1){ $num = $k1+1; echo $num.'、'.$v1.'<br>'; } ?>

                      </td>
                  <td><?php echo $v['job_time'];?></td>
                  <td class="td-status">
                  <?php if($v['job_show_status'] == '启用'):?>
                  ...
                  <td class="td-manage">
                      ...
                      <?php endif;?>
                      <a title="招聘修改" href="/apzd/apzd/index.php/Home/Employ/save/id/<?php echo $v['id'];?>"  class="btn btn-xs btn-info save" ><i class="icon-edit bigger-120"></i></a>
...
function member_stop(obj,id){
    layer.confirm('确认要停用吗？',function(index){
        $.ajax({
            type : 'post',
            url : '/apzd/apzd/index.php/Home/Employ/setShowStatus',
            data : {
                id : id,
                status : 0,
            },
...
function sort(obj,id){
    ...
    $.ajax({
        type : 'post',
        url : '/apzd/apzd/index.php/Home/Employ/sort',
        data : {
            num:num,id:id
        },
        dataType : 'json',
...
function member_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
        $.ajax({
            url : '/apzd/apzd/index.php/Home/Employ/delete/id/'+id,
            dataType : 'json',
            success : function(response){
